...
function Account() {
   	...
   	async function getFeedUnreadCount(params, rawFeed) {
        ...
        if (await storage.get("showOnlyRecentUnreadEmails")) {
            ...
            parsedFeed.querySelectorAll('entry').forEach(entry => {
                const entryDate = Date.parse(entry.querySelector('issued').textContent);
   ...
   this.syncSignInId = async function(secondCall) {
        ...
        if (response.ok) {
            if (response.url) { // Seems gmail will redirect to proper url with the /u/# so let's just extract it from the url
                const parts = response.url.match(/\/mail\/u\/(\d+)/);
                if (parts) {
                    const id = parts[1];
                    ...
                    if (emails?.first() == email) {
                        ...
                        const parts = data.match(/\/mail\/u\/(\d+)/);
                        const id = parts[1];
            ...
            if (fetchEmailsResponse.feedsArrayInfo) {
                fetchEmailsResponse.feedsArrayInfo.forEach(feedInfo => {
                    ...
                    parsedFeed.querySelectorAll('entry').forEach(entry => {
                        ...
                        const issued = Date.parse(entry.querySelector('issued').textContent);
   ...
   async function createMailObjects(params) {
	   ...
	   params.httpBodies.forEach(httpBody => {
   ...
   async function executeGmailHttpAction(params) {
        ...
        */
        ...
        const html = await ajaxBasicHTMLGmail(url, {
            options: options,
            ignoreResponse: !isReplyAction
        });
        ...
        if (accountAddingMethod == "autoDetect") {
            ...
            try {
                ...
                if (removedEmail && !params.instantlyUpdatedCount) {
                    ...
                    var newBadgeCount = await storage.get("unreadCount") - 1;
   ...
   async function getHtmlVar(key) {
        ...
        try {
            ...
            value = await fetchText(url, function(data) {
                const tmp = RegExp(`${key}\=\"([^\"]*)\"`).exec(data);
                if (tmp?.length >= 2) {
                    return tmp[1];
                }        
            });
        } catch (error) {
            throw `getHtmlVar ${key} error: ${error}`;
        }

        if (value) {
            if (value == "null") {
                throw `getHtmlVar ${key} is null`;
            } else {
                return value;
            }
        } else {
            throw `getHtmlVar ${key} not found`;
        }
   }

   async function fetchGmailAT2() {
        return getHtmlVar("GM_ACTION_TOKEN");
   }
   ...
   async function generateExecuteGmailAPIActionParams(params) {
       ...
       try {
            ...
            if (lastMessage) {
                replyQuoted = getMessage("onDATE_NAMEwrote", new Date(lastMessage.date).toLocaleStringJ(), lastMessage.from?.name) + "\n\n";
                replyQuoted += "> " + params.mail.getLastMessageText().replaceAll("\n", "\n> ");
            }
        } catch (error) {
		   ...
		   chrome.windows.update(params.tab.windowId, {focused:true}, function() {
               ...
               new Promise((resolve, reject) => {
                    if (params.label) { // ie. search
                        resolve(params.label);
                    } else {
                        resolve(that.getOpenLabel());
                    }
               }).then(label => {
