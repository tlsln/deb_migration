        ...
        if (alarm.name == Alarms.EVERY_MINUTE) {
            ...
            if (await isAnAccountCheckingSpam()) {
                ...
                if (await storage.get("accountAddingMethod") == "oauth") {
                    refreshAccounts({hardRefreshFlag: true});
                } else {
                    refreshAccounts();
                }
            }
...
function setBadgeEllipsis() {
	...
	if (chrome.browserAction.setBadgeTextColor) {
		chrome.browserAction.setBadgeTextColor({color: BadgeColor.WHITE});
	}
...
async function refreshAccounts(params = {}) {
    ...
    if (params.source != "popup") {
        chrome.runtime.sendMessage({command: "refreshPopup"});
    }
}
