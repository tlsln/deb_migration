    ...
    constructor() {
        ...
        this.getThread = async function (params = {}) {
            ...
            if (hasMessages || mail.account.getAccountAddingMethod() == "oauth") {
                ...
                let data = await fetchText(mail.account.getMailUrl({ useStandardGmailUrl: true, urlParams: `ui=2&view=pt&search=${searchValue}&th=${mail.id}`}))
                    ...
                    if (useMessages && that.messages.length > contributors.length + 1) {
                        ...
                        if (useMessages) {
                            if (contributors.length == 2) {
                                ...
                                span.textContent = contributors[nextContributorIndex].querySelector("name").textContent.split(" ")[0];
                                $node.append(", ", span);
                            }
                            else if (contributors.length >= 3) {
                                const span = document.createElement("span");
                                span.textContent = contributors[0].querySelector("name").textContent.split(" ")[0];
                                $node.append(" .. ", span);
                            }
                            $node.append(", ");
                        }
